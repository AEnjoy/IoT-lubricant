...
func (s *GatewayService) AddHost(ctx context.Context, info *model2.GatewayHost) error {
...
func (s *GatewayService) EditHost(ctx context.Context, hostid string, info *model2.GatewayHost) error {
	txn, errorCh, commit := s.txnHelper()
	defer commit()
...
func (s *GatewayService) GetHost(ctx context.Context, hostid string) (model2.GatewayHost, error) {
	return s.db.GetGatewayHostInfo(ctx, hostid)
}
func (s *GatewayService) UserGetHosts(ctx context.Context, userid string) ([]model2.GatewayHost, error) {
...
func (s *GatewayService) GetErrorLogs(ctx context.Context,
	gatewayid string, from, to time.Time, limit int) ([]model2.ErrorLogs, error) {
	if limit <= 0 {
		limit = 10
	}
	return s.db.GetErrorLogs(ctx, gatewayid, from, to, limit)
}
func (s *GatewayService) DescriptionError(ctx context.Context, errorID string) (model2.ErrorLogs, error) {
	return s.db.GetErrorLogByErrorID(ctx, errorID)
}

func (s *GatewayService) HostGetGatewayDeployConfig(ctx context.Context, hostid string) (*model2.ServerInfo, error) {
	hostInfo, err := s.db.GetGatewayHostInfo(ctx, hostid)
	if err != nil {
		err = exception.ErrNewException(err,
			exceptionCode.DbGetGatewayFailed,
...
func (s *GatewayService) GatewayGetGatewayDeployConfig(ctx context.Context, gatewayid string) (*model2.ServerInfo, error) {
	taskMq := s.store.Mq
	...
	var ret model2.ServerInfo
	err = sonic.Unmarshal((<-t).([]byte), &ret)
	return &ret, err
}

func (s *GatewayService) HostSetGatewayDeployConfig(ctx context.Context, hostid string, info *model2.ServerInfo) error {
	hostInfo, err := s.db.GetGatewayHostInfo(ctx, hostid)
	if err != nil {
		err = exception.ErrNewException(err,
			exceptionCode.DbGetGatewayFailed,
...
func (s *GatewayService) GatewaySetGatewayDeployConfig(ctx context.Context, gatewayid string, info *model2.ServerInfo) error {
	taskMq := s.store.Mq
