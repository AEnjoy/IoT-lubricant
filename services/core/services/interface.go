...
type IGatewayService interface {
	AddHost(ctx context.Context, info *model2.GatewayHost) error
	EditHost(ctx context.Context, hostid string, info *model2.GatewayHost) error
	GetHost(ctx context.Context, hostid string) (model2.GatewayHost, error)
	UserGetHosts(ctx context.Context, userid string) ([]model2.GatewayHost, error)
	...
	DescriptionError(ctx context.Context, errorID string) (model2.ErrorLogs, error)

	HostGetGatewayDeployConfig(ctx context.Context, hostid string) (*model2.ServerInfo, error)
	GatewayGetGatewayDeployConfig(ctx context.Context, gatewayid string) (*model2.ServerInfo, error)

	HostSetGatewayDeployConfig(ctx context.Context, hostid string, info *model2.ServerInfo) error
	GatewaySetGatewayDeployConfig(ctx context.Context, gatewayid string, info *model2.ServerInfo) error

	// internal
	AddHostInternal(ctx context.Context, info *model2.GatewayHost) error
